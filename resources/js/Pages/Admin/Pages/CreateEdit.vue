...
const form = useForm({
  ...
  seo: {
    ...
    og_image_url: props.page?.seo?.og_image_url || '',
    twitter_card: props.page?.seo?.twitter_card || 'summary_large_image',
    twitter_title: props.page?.seo?.twitter_title || '',
    twitter_description: props.page?.seo?.twitter_description || '',
    twitter_image: props.page?.seo?.twitter_image || ''
  }
})

// Block types helper
const sectionTypes = [
  { type: 'hero', label: 'Hero Banner', defaultData: { title: '', subtitle: '', primary_cta_text: '', primary_cta_url: '' } },
  { type: 'features', label: 'Features Grid', defaultData: { title: '', subtitle: '', items: [] } },
  { type: 'team', label: 'Team Members Showcase', defaultData: { title: 'Meet Our Experts', member_ids: [] } },
  { type: 'testimonials', label: 'Testimonials Slider', defaultData: { title: 'Client Reviews', testimonial_ids: [] } },
  { type: 'faqs', label: 'FAQ Accordions', defaultData: { title: 'Frequently Asked Questions', faq_ids: [] } },
  { type: 'content', label: 'Rich Body Content', defaultData: { body: '' } },
  { type: 'cta', label: 'Call To Action (CTA) Banner', defaultData: { title: 'Ready to elevate your agency brand?', button_text: 'Get Started', button_url: '' } }
]
...
// Helpers for features section items
...
<template>
  <Head :title="isEditing ? 'Edit CMS Page - Admin' : 'Create CMS Page - Admin'" />
  ...
  <AdminLayout>
    <form @submit.prevent="submit" class="space-y-6 max-w-6xl mx-auto pb-16">
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900 border border-slate-800/80 p-5 rounded-2xl">
        <div class="flex-1 w-full">
          <div class="text-xs font-semibold text-indigo-400 uppercase tracking-widest mb-1.5">
            {{ isEditing ? 'Edit Page Details' : 'Design Dynamic CMS Page' }}
          </div>
          <input 
            v-model="form.title" 
            type="text" 
      ...
      <!-- Tab Navigation and Slug Settings -->
      <div class="grid md:grid-cols-3 gap-6">
        <!-- Main Form & Settings Sidebar -->
        <div class="md:col-span-1 space-y-6">
          <!-- Slug Config Card -->
          <div class="bg-slate-900 border border-slate-800/80 p-5 rounded-2xl space-y-4">
            <h3 class="text-xs font-bold uppercase tracking-widest text-slate-400">Route URL Settings</h3>
            <div>
              ...
              <div class="flex rounded-xl overflow-hidden border border-slate-800 bg-slate-950">
                <span class="px-3 py-2.5 bg-slate-900 text-slate-500 text-xs border-r border-slate-800">/</span>
                <input 
                  v-model="form.slug" 
                  type="text" 
                  placeholder="about-us" 
                  class="bg-transparent border-0 focus:ring-0 flex-1 px-3 py-2 text-xs text-slate-200 placeholder-slate-650 focus:outline-none"
                />
              </div>
              <p class="text-[10px] text-slate-500 mt-1.5">Leave blank to automatically auto-slugify the title.</p>
          ...
          <div v-if="activeTab === 'content'" class="bg-slate-900 border border-slate-800/80 p-5 rounded-2xl space-y-3">
            <h3 class="text-xs font-bold uppercase tracking-widest text-slate-400">Add Layout Blocks</h3>
            <p class="text-[10px] text-slate-500 leading-normal mb-2">Build your page layout by adding blocks and editing contents.</p>
        ...
        <!-- Dynamic Content Editor Area -->
        <div class="md:col-span-2 space-y-6">
          <!-- BLOCK EDITOR TAB -->
          <div v-if="activeTab === 'content'" class="space-y-4">
            <div class="flex items-center justify-between">
              <h2 class="text-sm font-bold uppercase tracking-widest text-slate-500">Layout Builder Sequence</h2>
            ...
            <div v-if="form.content.length === 0" class="bg-slate-900/40 border border-dashed border-slate-800 rounded-3xl p-16 text-center text-slate-500">
              ...
              <p class="text-xs text-slate-600 max-w-sm mx-auto">Select layout blocks from the sidebar picker to begin structuring this dynamic page.</p>
            </div>
            ...
            <div 
              ...
              :key="idx" 
              class="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-lg transition duration-200 hover:border-slate-850"
            >
              ...
              <div class="bg-slate-950 px-5 py-3.5 border-b border-slate-800/80 flex items-center justify-between">
                ...
                <div class="flex items-center gap-1">
                  <!-- Up/Down control -->
                  <button type="button" @click="moveSectionUp(idx)" :disabled="idx === 0" class="p-1 rounded text-slate-500 hover:text-slate-200 hover:bg-slate-850 disabled:opacity-30">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7" />
                    </svg>
                  </button>
                  <button type="button" @click="moveSectionDown(idx)" :disabled="idx === form.content.length - 1" class="p-1 rounded text-slate-500 hover:text-slate-200 hover:bg-slate-850 disabled:opacity-30">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <!-- Delete block -->
                  <button type="button" @click="removeSection(idx)" class="p-1 rounded text-rose-500 hover:text-rose-400 hover:bg-rose-950/20">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div class="p-5 space-y-4 bg-slate-900/40">
                ...
                <div v-if="section.type === 'hero'" class="grid sm:grid-cols-2 gap-4">
                  <div class="sm:col-span-2">
                    <label class="admin-label">Hero Main Title</label>
                  ...
                  <div>
                    ...
                    <input v-model="section.data.primary_cta_text" type="text" class="admin-input" placeholder="Get Started" />
                  </div>
                  <div>
                    <label class="admin-label">Primary Button URL</label>
                    <input v-model="section.data.primary_cta_url" type="text" class="admin-input" placeholder="/contact-us" />
                  </div>
                </div>
                ...
                <div v-else-if="section.type === 'features'" class="space-y-4">
                  <div class="grid sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <input v-model="section.data.title" type="text" class="admin-input" placeholder="Our Core Features" />
                    </div>
                    <div>
                      <label class="admin-label">Section Subtitle</label>
                      <input v-model="section.data.subtitle" type="text" class="admin-input" placeholder="Why clients select us" />
                    </div>
                  </div>
                  
                  <div class="space-y-2">
                    <div class="flex justify-between items-center">
                      <span class="text-xs font-semibold text-slate-400">Features Grid List Items</span>
                    ...
                    <div v-for="(item, iIdx) in section.data.items" :key="iIdx" class="bg-slate-950 p-4 border border-slate-800 rounded-xl space-y-3 relative">
                      ...
                      <div class="grid sm:grid-cols-3 gap-3">
                        <div class="sm:col-span-1">
                          ...
                          <input v-model="item.title" type="text" class="admin-input" placeholder="Quality First" required />
                        </div>
                        <div class="sm:col-span-2">
                          <label class="admin-label">Feature Description</label>
                          <input v-model="item.description" type="text" class="admin-input" placeholder="Brief description details..." required />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div v-else-if="section.type === 'team'" class="space-y-3">
                  <div>
                    <label class="admin-label">Team Section Title</label>
                    <input v-model="section.data.title" type="text" class="admin-input" placeholder="Meet The Team" />
                  </div>
                  <div>
                    <label class="admin-label">Select Members to Include</label>
                    <div class="grid sm:grid-cols-2 gap-2 mt-2 max-h-48 overflow-y-auto p-3 bg-slate-950 rounded-xl border border-slate-800">
                      ...
                      >
                        <input 
                          type="checkbox" 
                          :checked="section.data.member_ids.includes(member.id)"
                          @change="toggleIdSelection(section.data.member_ids, member.id)"
                ...
                <div v-else-if="section.type === 'testimonials'" class="space-y-3">
                  <div>
                    ...
                    <input v-model="section.data.title" type="text" class="admin-input" placeholder="Client Success Stories" />
                  </div>
                  <div>
                ...
                <div v-else-if="section.type === 'content'">
                  <label class="admin-label">Body Text (HTML / Plaintext)</label>
                  <textarea 
                    v-model="section.data.body" 
                    rows="6" 
                    class="admin-input font-mono text-xs" 
                    placeholder="Enter prose markup content here..."
                    required
                  ></textarea>
                </div>
                ...
                <div v-else-if="section.type === 'cta'" class="grid sm:grid-cols-2 gap-4">
                  <div class="sm:col-span-2">
                    <label class="admin-label">CTA Message Header</label>
                  ...
                  <div>
                    ...
                    <input v-model="section.data.button_text" type="text" class="admin-input" placeholder="Get In Touch" required />
                  </div>
                  <div>
                    <label class="admin-label">CTA Action URL</label>
          ...
          <div v-else-if="activeTab === 'seo'" class="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-6">
            <div>
              ...
              <p class="text-[10px] text-slate-500">Provide search engine optimized tags for this dynamic CMS page.</p>
            </div>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input v-model="form.seo.canonical_url" type="url" class="admin-input" placeholder="https://agency.com/about" />
              </div>
            </div>
