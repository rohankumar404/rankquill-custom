...
const initialCategoryIds = props.portfolio?.categories ? props.portfolio.categories.map(c => c.id) : []

const form = useForm({
  client_name: props.portfolio?.client_name || '',
  project_title: props.portfolio?.project_title || '',
  slug: props.portfolio?.slug || '',
  challenge: props.portfolio?.challenge || '',
  approach: props.portfolio?.approach || '',
  results: props.portfolio?.results || '',
  metrics: props.portfolio?.metrics || [],
  before_image_path: props.portfolio?.before_image_path || '',
  after_image_path: props.portfolio?.after_image_path || '',
  gallery: props.portfolio?.gallery || [],
  video_url: props.portfolio?.video_url || '',
  completed_at: props.portfolio?.completed_at ? props.portfolio.completed_at.substring(0, 10) : '',
  status: props.portfolio?.status || 'draft',
  category_ids: initialCategoryIds,
  content: props.portfolio?.content || [],
  seo: {
    meta_title: props.portfolio?.seo?.meta_title || '',
    meta_description: props.portfolio?.seo?.meta_description || '',
    meta_keywords: props.portfolio?.seo?.meta_keywords || '',
    robots: props.portfolio?.seo?.robots || 'index, follow',
    canonical_url: props.portfolio?.seo?.canonical_url || '',
    og_title: props.portfolio?.seo?.og_title || '',
    og_description: props.portfolio?.seo?.og_description || '',
    og_image_url: props.portfolio?.seo?.og_image_url || '',
    twitter_card: props.portfolio?.seo?.twitter_card || 'summary_large_image',
    twitter_title: props.portfolio?.seo?.twitter_title || '',
    twitter_description: props.portfolio?.seo?.twitter_description || '',
    twitter_image: props.portfolio?.seo?.twitter_image || ''
  }
})
...
<template>
  <Head :title="isEditing ? 'Edit Case Study - Admin' : 'Add Case Study - Admin'" />
      ...
      <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-slate-900 border border-slate-800/80 p-5 rounded-2xl">
        <div class="flex-1 w-full">
          <div class="text-xs font-semibold text-indigo-400 uppercase tracking-widest mb-1.5">
            {{ isEditing ? 'Edit Portfolio Case' : 'Register New Case Study' }}
          </div>
          <input 
            v-model="form.project_title" 
            type="text" 
            placeholder="Project Title (e.g. 300% Organic Traffic Growth)" 
            ...
            required
          />
          <span v-if="form.errors.project_title" class="text-xs text-rose-500 block mt-1">{{ form.errors.project_title }}</span>
        </div>
        ...
        <div class="flex items-center gap-3 w-full md:w-auto justify-end">
          ...
          >
            {{ form.processing ? 'Saving...' : 'Save Case Study' }}
          </button>
        </div>
      </div>
      ...
      <div class="grid md:grid-cols-3 gap-6">
        ...
        <div class="md:col-span-1 space-y-6">
          ...
          <div class="bg-slate-900 border border-slate-800/80 p-5 rounded-2xl space-y-4">
            <h3 class="text-xs font-bold uppercase tracking-widest text-slate-400">Case Routing & Info</h3>
            ...
            <div>
              ...
              <input 
                v-model="form.slug" 
                type="text" 
                placeholder="ecommerce-seo-case-study" 
                class="admin-input"
              />
              <p class="text-[10px] text-slate-500 mt-1">Leave blank to auto-slugify the project title.</p>
              <span v-if="form.errors.slug" class="text-xs text-rose-500 block mt-1">{{ form.errors.slug }}</span>
            </div>

            <!-- Client Name -->
            <div>
              <label class="admin-label">Client / Brand Name</label>
              <input v-model="form.client_name" type="text" placeholder="Acme Corp" class="admin-input" required />
              <span v-if="form.errors.client_name" class="text-xs text-rose-500 block mt-1">{{ form.errors.client_name }}</span>
            </div>

            <!-- Completion date -->
            <div>
              <label class="admin-label">Project Completed Date</label>
              <input v-model="form.completed_at" type="date" class="admin-input" />
              <span v-if="form.errors.completed_at" class="text-xs text-rose-500 block mt-1">{{ form.errors.completed_at }}</span>
            </div>
          </div>

          <!-- Category Selection -->
          <div class="bg-slate-900 border border-slate-800/80 p-5 rounded-2xl space-y-3">
            <h3 class="text-xs font-bold uppercase tracking-widest text-slate-400">Case Categories</h3>
          ...
          <!-- Section Picker (Only when content tab active) -->
        ...
        <div class="md:col-span-2 space-y-6">
          ...
          <div v-if="activeTab === 'general'" class="space-y-6">
            ...
            <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-xl">
              <div class="flex justify-between items-center">
                <h3 class="text-xs font-bold uppercase tracking-widest text-slate-200">KPI Performance Metrics</h3>
                <button type="button" @click="addMetric" class="text-[10px] text-indigo-400 font-bold hover:underline">+ Add KPI Metric</button>
              </div>

              <div class="space-y-3">
                <div v-for="(metric, mIdx) in form.metrics" :key="mIdx" class="grid sm:grid-cols-2 gap-3 bg-slate-950 p-4 rounded-xl border border-slate-850 relative">
                  <button type="button" @click="removeMetric(mIdx)" class="absolute top-2 right-2 text-rose-500 hover:text-rose-400 text-xs">
                    Remove
                  </button>
                  <div>
                    <label class="admin-label">Metric Value (e.g. +340%)</label>
                    <input v-model="metric.value" type="text" class="admin-input" placeholder="e.g. +340%" required />
                  </div>
                  <div>
                    <label class="admin-label">Metric Label (e.g. Traffic Growth)</label>
                    <input v-model="metric.label" type="text" class="admin-input" placeholder="e.g. Organic Clicks" required />
                  </div>
                </div>
                <div v-if="form.metrics.length === 0" class="text-center text-slate-650 text-xs italic py-4">No performance metrics added yet.</div>
              </div>
            </div>

            <!-- Narrative Textarea Card -->
            <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-5 shadow-xl">
              <h3 class="text-xs font-bold uppercase tracking-widest text-slate-200">Project Narratives</h3>
              
              <!-- Challenge -->
              <div class="space-y-1.5">
                <label class="admin-label">The Challenge</label>
                <textarea v-model="form.challenge" rows="4" class="admin-input" placeholder="What was the client's original problem..."></textarea>
                <span v-if="form.errors.challenge" class="text-xs text-rose-500 block mt-1">{{ form.errors.challenge }}</span>
              </div>

              <!-- Approach -->
              <div class="space-y-1.5">
                <label class="admin-label">Our Solution / Approach</label>
                <textarea v-model="form.approach" rows="4" class="admin-input" placeholder="How our team solved it, what strategies we implemented..."></textarea>
                <span v-if="form.errors.approach" class="text-xs text-rose-500 block mt-1">{{ form.errors.approach }}</span>
              </div>

              <!-- Results -->
              <div class="space-y-1.5">
                <label class="admin-label">The Outcomes / Results</label>
                <textarea v-model="form.results" rows="4" class="admin-input" placeholder="What was the final impact on rankings, leads, or revenue..."></textarea>
                <span v-if="form.errors.results" class="text-xs text-rose-500 block mt-1">{{ form.errors.results }}</span>
              </div>
            </div>

            <!-- Media Links Card -->
            <div class="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-4 shadow-xl">
              <h3 class="text-xs font-bold uppercase tracking-widest text-slate-200">Media Assets & Comparison</h3>
              
              <div class="grid sm:grid-cols-2 gap-4">
                <div>
                  <label class="admin-label">Before Screenshot Path</label>
                  <input v-model="form.before_image_path" type="text" placeholder="/storage/media/before.png" class="admin-input" />
                </div>
                <div>
                  <label class="admin-label">After Screenshot Path</label>
                  <input v-model="form.after_image_path" type="text" placeholder="/storage/media/after.png" class="admin-input" />
                </div>
                <div class="sm:col-span-2">
                  <label class="admin-label">Video Case Study URL</label>
                  <input v-model="form.video_url" type="text" placeholder="https://youtube.com/watch?v=..." class="admin-input" />
                </div>
              </div>
              ...
              <div class="border-t border-slate-800/80 pt-4 space-y-2">
                <div class="flex justify-between items-center">
                  <span class="text-xs font-semibold text-slate-400">Additional Gallery Screenshots</span>
                  <button type="button" @click="addGalleryItem" class="text-[10px] text-indigo-400 font-bold hover:underline">+ Add Screenshot</button>
                </div>
                <div class="space-y-2">
                  <div v-for="(img, idx) in form.gallery" :key="idx" class="flex gap-2 items-center">
                    <input v-model="form.gallery[idx]" type="text" class="admin-input" placeholder="/storage/media/screenshot1.png" required />
                    <button type="button" @click="removeGalleryItem(idx)" class="text-rose-550 hover:text-rose-500 text-xs px-2 py-1">✕</button>
                  </div>
                  <div v-if="form.gallery.length === 0" class="text-center text-slate-650 text-xs italic py-2">No gallery screenshots added.</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div v-else-if="activeTab === 'content'" class="space-y-4">
            <div class="flex items-center justify-between">
              <h2 class="text-sm font-bold uppercase tracking-widest text-slate-500">Dynamic Case Blocks</h2>
            ...
            <div v-if="form.content.length === 0" class="bg-slate-900/40 border border-dashed border-slate-800 rounded-3xl p-16 text-center text-slate-500">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 mx-auto text-slate-700 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10" />
              </svg>
              <h4 class="text-slate-300 font-semibold text-sm mb-1">Canvas is empty</h4>
              <p class="text-xs text-slate-600 max-w-sm mx-auto">Select layout blocks from the sidebar picker to begin structuring the case study page.</p>
            </div>
            ...
            >
              <!-- Section Control Bar -->
              <div class="bg-slate-955 px-5 py-3.5 border-b border-slate-800/80 flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <span class="text-[10px] font-mono font-bold bg-slate-850 border border-slate-750 px-2 py-0.5 rounded text-indigo-405 uppercase">
                    {{ section.type }}
                  </span>
              ...
              <div class="p-5 space-y-4 bg-slate-900/40">
                ...
                <div v-if="section.type === 'hero'" class="grid sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label class="admin-label">CTA URL</label>
                  ...
                  <div class="sm:col-span-2">
                    ...
                    <input v-model="section.data.image" type="text" class="admin-input" placeholder="https://agency.com/images/case-banner.jpg" />
                  </div>
                </div>
                ...
                <div v-else-if="section.type === 'features'" class="space-y-4">
                  <div>
                    ...
                    <input v-model="section.data.title" type="text" class="admin-input" placeholder="Key Achievements" />
                  </div>
                  
                  <div class="space-y-2">
                    <div class="flex justify-between items-center">
                      <span class="text-xs font-semibold text-slate-400">Features List Items</span>
                      <button type="button" @click="addFeatureItem(idx)" class="text-[10px] text-indigo-400 font-bold hover:underline">+ Add Item</button>
                    </div>
                    ...
                    <div v-for="(item, iIdx) in section.data.items" :key="iIdx" class="bg-slate-955 p-4 border border-slate-805 rounded-xl space-y-3 relative">
                      ...
                      <div class="grid sm:grid-cols-3 gap-3">
                        <div class="sm:col-span-1">
                          ...
                          <input v-model="item.title" type="text" class="admin-input" placeholder="Deliverable" required />
                        </div>
                        <div class="sm:col-span-2">
                          <label class="admin-label">Feature Description</label>
                          <input v-model="item.description" type="text" class="admin-input" placeholder="Describe the outcome details..." required />
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <div v-else-if="section.type === 'testimonials'" class="space-y-3">
                  <div>
                    ...
                    <input v-model="section.data.title" type="text" class="admin-input" placeholder="Client Reviews" />
                  </div>
                  <div>
                    <label class="admin-label">Select Testimonials</label>
                ...
                <div v-else-if="section.type === 'faqs'" class="space-y-3">
                  <div>
                    ...
                    <input v-model="section.data.title" type="text" class="admin-input" placeholder="Frequently Asked Questions" />
                  </div>
                  <div>
                    <label class="admin-label">Select FAQs</label>
                    <div class="grid sm:grid-cols-2 gap-2 mt-2 max-h-48 overflow-y-auto p-3 bg-slate-950 rounded-xl border border-slate-800">
                      <label 
                        v-for="faq in faqs" 
                        :key="faq.id"
                ...
                <div v-else-if="section.type === 'content'">
                  <label class="admin-label">Prose content block (HTML / Text)</label>
                  <textarea 
                ...
                <div v-else-if="section.type === 'cta'" class="grid sm:grid-cols-2 gap-4">
                  <div class="sm:col-span-2">
                    ...
                    <input v-model="section.data.title" type="text" class="admin-input" placeholder="Ready to scale your business?" required />
                  </div>
                  <div class="sm:col-span-2">
          ...
          <div v-else-if="activeTab === 'seo'" class="bg-slate-900 border border-slate-800 p-6 rounded-2xl space-y-6">
            <div>
              ...
              <p class="text-[10px] text-slate-500">Provide search engine optimized tags for this portfolio case study.</p>
            </div>
            ...
            <div class="grid sm:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input v-model="form.seo.canonical_url" type="url" class="admin-input" placeholder="https://agency.com/portfolio/slug" />
              </div>
            </div>
