...
<template>
  ...
  <div class="min-h-screen bg-slate-950 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white">
    <!-- Preview Banner -->
    <div v-if="isPreview" class="bg-gradient-to-r from-amber-600 to-amber-500 text-slate-950 font-semibold px-4 py-2 text-center text-sm shadow-md flex items-center justify-center gap-2">
    ...
    <header class="border-b border-slate-800/80 bg-slate-900/40 backdrop-blur-md sticky top-0 z-40">
      <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-8 text-sm font-medium text-slate-300">
          ...
          <a href="#" class="hover:text-indigo-400 transition-colors text-indigo-400">Pages</a>
          <a href="#" class="hover:text-indigo-400 transition-colors">Blog</a>
        </nav>
      ...
      <div v-for="(section, idx) in page.content" :key="idx" class="relative">
        ...
        <section v-if="section.type === 'hero'" class="max-w-7xl mx-auto px-6 py-20 md:py-32 grid md:grid-cols-2 gap-12 items-center">
          ...
          <div v-if="section.data.image" class="relative">
            ...
            <img :src="section.data.image" alt="Hero Image" class="relative rounded-2xl border border-slate-800 shadow-2xl object-cover w-full aspect-video" />
          </div>
        </section>

        <!-- CTA Section -->
        <section v-else-if="section.type === 'cta'" class="max-w-5xl mx-auto px-6 py-16 my-16 bg-gradient-to-br from-indigo-950/80 to-purple-950/40 border border-slate-800/80 rounded-3xl p-12 text-center relative overflow-hidden shadow-2xl">
          <div class="absolute -top-32 -left-32 w-96 h-96 bg-indigo-500 rounded-full blur-3xl opacity-10"></div>
          <div class="relative z-10 space-y-6">
            <h2 class="text-3xl md:text-5xl font-bold text-white tracking-tight">{{ section.data.title }}</h2>
            <p class="text-slate-300 max-w-2xl mx-auto font-light leading-relaxed">{{ section.data.subtitle }}</p>
            <div class="pt-4">
              <a :href="section.data.button_url" class="inline-block bg-white hover:bg-slate-100 text-slate-950 font-semibold px-8 py-4 rounded-full transition-all duration-300 shadow-lg shadow-white/10 hover:scale-[1.02]">
        ...
        <section v-else-if="section.type === 'faqs'" class="max-w-4xl mx-auto px-6 py-16">
          <h2 class="text-3xl font-bold text-center mb-12 text-white">{{ section.data.title || 'Frequently Asked Questions' }}</h2>
          <div class="space-y-4">
            <div v-for="faq in section.data.resolved_items" :key="faq.id" class="border border-slate-800 bg-slate-900/30 rounded-2xl p-6 hover:border-slate-700/80 transition-colors">
              <h3 class="text-lg font-semibold text-white mb-2">{{ faq.question }}</h3>
              <p class="text-slate-400 font-light leading-relaxed">{{ faq.answer }}</p>
            </div>
          </div>
        </section>
        ...
        <section v-else-if="section.type === 'testimonials'" class="max-w-7xl mx-auto px-6 py-16">
          <h2 class="text-3xl font-bold text-center mb-12 text-white">{{ section.data.title || 'Client Success Stories' }}</h2>
          <div class="grid md:grid-cols-3 gap-8">
            <div v-for="t in section.data.resolved_items" :key="t.id" class="border border-slate-800 bg-slate-900/20 p-8 rounded-2xl flex flex-col justify-between hover:border-indigo-500/30 transition-all duration-300">
              <p class="text-slate-300 italic mb-6">"{{ t.testimonial }}"</p>
              <div class="flex items-center gap-4">
                <div v-if="t.avatar_path" class="w-12 h-12 rounded-full overflow-hidden border border-slate-700">
                  <img :src="t.avatar_path" alt="Client Avatar" class="object-cover w-full h-full" />
                </div>
                <div>
                  <h4 class="font-semibold text-white">{{ t.client_name }}</h4>
                  <p class="text-xs text-slate-500">{{ t.client_position }}, {{ t.client_company }}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <section v-else-if="section.type === 'team'" class="max-w-7xl mx-auto px-6 py-16">
          <h2 class="text-3xl font-bold text-center mb-12 text-white">{{ section.data.title || 'Meet Our Experts' }}</h2>
          <div class="grid sm:grid-cols-2 md:grid-cols-4 gap-8">
            <div v-for="m in section.data.resolved_items" :key="m.id" class="group text-center">
              <div class="relative overflow-hidden rounded-2xl border border-slate-800 aspect-square mb-4 bg-slate-900">
              ...
              <p class="text-sm text-indigo-400">{{ m.position }}</p>
            </div>
          </div>
        </section>
        ...
        <section v-else-if="section.type === 'features'" class="max-w-7xl mx-auto px-6 py-16">
          <div class="grid md:grid-cols-3 gap-8">
            <div v-for="(item, i) in section.data.items" :key="i" class="border border-slate-800 p-8 rounded-2xl hover:border-slate-700/80 transition-colors">
              <h3 class="font-bold text-xl text-white mb-2">{{ item.title }}</h3>
              <p class="text-slate-400 font-light text-sm leading-relaxed">{{ item.description }}</p>
            </div>
          </div>
        </section>

        <!-- Rich Text Content Section -->
        <section v-else-if="section.type === 'content'" class="max-w-3xl mx-auto px-6 py-16 prose prose-invert prose-indigo">
